...
const SearchInput = () => {
    ...
    return (
        <div
            ...
            className=" relative flex h-fit w-auto space-x-1 rounded text-xs text-white shadow "
        >
            <div className="relative hidden ease-in-out md:flex md:transition-transform">
                <input
                    ...
                    className="h-8 w-full bg-zinc-300 text-xs text-black focus:border-white focus:outline-none"
                />
            </div>
            <Link
                href={`/search?keyword=${searchTerm}`}
                className=" mx-auto h-full w-fit border-s border-white px-2 py-1"
            >
                <svg
